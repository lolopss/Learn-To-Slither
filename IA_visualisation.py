...
def compute_direction_scan(game):
    ...
    for dx, dy in directions:
        ...
        imm.append(int(nx < 0 or nx >= size or ny < 0 or ny >= size or
                       (nx, ny) in body))
        green_seen = 0
        red_seen = 0
        body_seen = 0
...
def draw_state_panel(screen, game, info, panel_x, panel_y,
    ...
    screen.blit(title, (panel_x + (width - title.get_width()) //
                        2, panel_y + 6))
    grid_x = panel_x + 8
    ...
    for d_i, label in enumerate(DIRECTION_LABELS):
        ...
        pygame.draw.rect(screen, hdr_col, (hx, grid_y, col_w - 6,
                                           header_h - 6), border_radius=4)
    ...
    for r_i, (row_key, values) in enumerate(info["rows"]):
        ...
        if row_key == "Free":
            ...
            if positives:
                best_idx = positives[0]
            else:
                best_idx = int(
                    max(range(4), key=lambda i: values[i])
                )
    ...
    if action_values is not None:
        ...
        screen.blit(q_title, (panel_x + (width -
                    q_title.get_width()) // 2, q_y + 4))

        # Exploration flag
        if next_is_explore:
            exp_font = pygame.font.SysFont(None, 22)
            exp_txt = exp_font.render("EXPLORATION", True, (255, 60, 60))
            screen.blit(exp_txt, (panel_x + (width - exp_txt.get_width())
                        // 2, q_y + 28))
        ...
        for i, (n, qv) in enumerate(zip(names, action_values)):
            ...
            by = q_y + 40
            act_col = (
                (70, 130, 180) if i == 0 else
                (205, 133, 63) if i == 1 else
            ...
            if highlight:
                base_draw_col = tuple(min(255, int(c * 1.7))
                                      for c in base_draw_col)
            ...
            screen.blit(at, (bx + 35 - at.get_width()
                        // 2, by - at.get_height() // 2))
